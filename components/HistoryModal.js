...
import { perChallanBalances } from "@/lib/calculations";
...
export default function HistoryModal({ challan, allTransactions, onClose }) {
  ...
  const balances = perChallanBalances(allTransactions, challan);
  const sample = rows[0] || {};
              ...
              {rows.map((t) => (
                <Fragment key={t.id}>
                  ...
                  {t.remarks && (
                    <tr>
                      <td></td>
                      <td colSpan={5} className="text-[12px] text-ink-faint !pt-0 !pb-2.5">
                        — {t.remarks}
                      </td>
                    </tr>
                  )}
